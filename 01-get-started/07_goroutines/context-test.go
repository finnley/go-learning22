...
func memberInfo5(ctx context.Context) {
	...
	for {
		select {
		case <-ctx.Done():
			fmt.Println("监控Memory退出")
			return
		default:
			time.Sleep(time.Second)
			fmt.Println("获取Memory成功")
		}
	}
}
...
func main() {
	...
	//// 使用channel的方式
	//wg.Add(1)
	//// 使用WithCancel生成一个context对象，里面传入的是父Context，通常是个固定写法，通过context.Background()生成父Context
	//// 然后根据父context对象生成一个子的context对象，同时返回一个 cancel 函数
	//// 生成后的子context需要传递给子goroutine
	//// 传递后随时可以通过cancel函数来执行，就知道可以取消了
	//// 在执行函数的时候又是怎么知道取消的请求呢？也是使用select，直接使用ctx.Done()方法就可以知道是否取消了
	//// ctx.Done()可以知道外部是否发送了中断请求
